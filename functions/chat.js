exports.handler = async function(event, context) {
  if (event.httpMethod !== "POST") {
    return { statusCode: 405, body: JSON.stringify({ error: "Sadece POST destekleniyor" }) };
  }
  let body;
  ...
  const { message } = body;
  if (!message) return { statusCode: 400, body: JSON.stringify({ error: "Mesaj boş" }) };
  const messages = [
  ...
  try {
    const groqRes = await fetch("https://api.groq.com/openai/v1/chat/completions", {
      method: "POST",
      headers: { "Authorization": "Bearer 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", "Content-Type": "application/json" },
      body: JSON.stringify({ model: "llama-3.1-8b-instant", messages, max_tokens: 800 })
    });
    const data = await groqRes.json();
    const reply = data.choices?.[0]?.message?.content;
    if (!reply) return { statusCode: 500, body: JSON.stringify({ error: "Groq cevap vermedi" }) };
